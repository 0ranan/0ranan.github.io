---
title: Git常用命令
date: 2021/11/17 16:49:07
updated: 2026/07/21 16:49:07
comments: true
tags:
  - 编程
  - git
categories:
typora-root-url: ..
---

# Git常用命令

> 参考
>
> [Git教程 - 廖雪峰的官方网站 (liaoxuefeng.com)](https://www.liaoxuefeng.com/wiki/896043488029600/)
>
> [Git 教程 | 菜鸟教程 (runoob.com)](https://www.runoob.com/git/git-tutorial.html)
>
> [git push 报错为fatal: The current branch master has no upstream branch. To push the current branch and_小鼠2021的博客-CSDN博客](https://blog.csdn.net/qq_45279570/article/details/107094799)

## 将一个远程仓库同步下来并且更新

```shell
git clone 远程仓库地址
cd 仓库名称
git add .
git commit -m "commit"
git push -u origin master
```

## git本地仓库初始化

```shell
git init #git仓库初始化
git add . #添加所有变化的文件
git commit -m “first commit” #提交到本地仓库
git remote add origin 远程仓库地址 #与远程仓库绑定
git push -u origin master #提交到远程仓库主分支
```

### 绑定远程仓库

```shell
git remote add test git@gitee.com:zhz000/test.git
```

### clone#克隆

```
git clone 远程仓库地址
```

### Git Branch 创建新分支

> 创建新的分支

```
git branch 分支名
```

```
强制修改分支位置
git branch -f main HEAD~3
```

### git checkout 切换分支

> 后面加上~相对引用，~[数字相对引用的层数]

```shell
git checkout 分支名
```


### git merge 合并分支

```
git merge 被合并的分支
```

### git rebase 提取提交到当前分支
```
git rebase 被提取的分支
```

### 查看git 日志

```
git log
```

### 查看git分支

```
git branch
```



### 强制推送

```
git push --force
```

## Git Submodule 使用

### 添加子模块

在根目录执行以下命令添加子模块：

```shell
# 1. 添加子模块（路径可按团队约定调整）
git submodule add <子模块仓库地址> <子模块路径>

# 2. 确认生成了 .gitmodules，并检出了内容
cat .gitmodules
ls <子模块路径>

# 3. 提交（会同时提交 .gitmodules 和子模块指针）
git add .gitmodules <子模块路径>
git commit -m "$(cat <<'EOF'
Add <子模块名称> as git submodule.
EOF
)"
```

### 克隆时初始化子模块

其他人克隆主仓库后需要初始化子模块：

```shell
# 新克隆时一并拉取
git clone --recurse-submodules <主仓库地址>

# 已有主仓库时补拉子模块
git submodule update --init --recursive
```

### 日常更新子模块

更新子模块到远端最新：

```shell
cd <子模块路径>
git fetch
git checkout <目标分支或 tag>   # 例如 main / master
cd ..
git add <子模块路径>
git commit -m "Bump <子模块名称> submodule"
```

## git 官方文件翻译

```
用法: git [--version] [--help] [-C <path>] [-c <name>=<value>]
           [--exec-path[=<path>]] [--html-path] [--man-path] [--info-path]
           [-p | --paginate | -P | --no-pager] [--no-replace-objects] [--bare]
           [--git-dir=<path>] [--work-tree=<path>] [--namespace=<name>]
           <command> [<args>]

这些是在各种情况下使用的常见Git命令：

开始一个工作区域（另请参见：git帮助教程）
   clone     将存储库克隆到新目录中
   init     创建一个空的Git存储库或重新初始化现有的存储库


处理当前更改（另请参阅：git帮助每日）
   add       将文件内容添加到索引
   mv        移动或重命名文件、目录或符号链接
   restore   还原工作树文件
   rm        从工作树和索引中删除文件

检查历史记录和状态（另请参阅：git帮助修订版）
   bisect    使用二进制搜索查找引入错误的提交
   diff      显示提交、提交和工作树等之间的更改
   grep      打印与图案匹配的线条
   log       显示提交日志
   show      显示各种类型的对象
   status    显示工作树状态


成长、标记和调整您的共同历史
   branch    列表、创建或删除分支
   commit    记录对存储库的更改
   merge     将两个或多个开发历史合并在一起
   rebase    在另一个基础提示之上重新应用提交
   reset     将当前磁头复位到指定状态
   switch    交换分支
   tag      创建、列出、删除或验证用GPG签名的标记对象




协作（另请参见：git帮助工作流）
   fetch     从另一个存储库下载对象和引用
   pull      从另一个存储库或本地分支获取并与之集成
   push     更新远程参照以及相关对象


“git help-a”和“git help-g”列出了可用的子命令和一些

概念指南。请参阅“git帮助＜命令＞”或“git帮助＜概念＞”

阅读有关特定子命令或概念的信息。

有关系统的概述，请参阅“git help git”。

```

